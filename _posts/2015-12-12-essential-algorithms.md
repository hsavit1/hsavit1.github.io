...
- Randomizing Data: By "randomizing data," the book mean seeing how a program would fare with any random input. A great example of this is a Monte Carlo simulation. It's actually hard to get truly random values (hisenberg's uncertainty principle and quantum mechanics confirm that). We use a process called a _liner congruential generator_ to generate random values and are mostly satisfied that the values are random enough.
